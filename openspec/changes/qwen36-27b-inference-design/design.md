...
| `spec.vllmVersion=0.18.0` 或 `0.19.x.rcx` | Qwen3.6 候选版本组合 | `netrsnpython3rdadvance` | Qwen3.6-27B |

Qwen3.6 候选版本线当前以 0.18.0 为基准设计；若产品最终选择 0.19.x.rcx，则路由、打包、安全扫描和性能验收要求保持不变，仅替换 vLLM/vllm-ascend 版本组合与镜像标签。
...
2. 在灰度环境按 CR `spec.vllmVersion=0.18.0` 或 `0.19.x.rcx` 路由 Qwen3.6 实例，保留 `0.13.0` 回退路径。
3. 先启用 FP16 eager，随后逐项启用 W8 动态量化、Chunk Prefill、ACLGraph、Prefix Cache、MTP；每次只打开一个新增变量。
4. 若 Qwen3.6 候选版本线运行态依赖或性能不达标，CR 回退到不承载 Qwen3.6 的 0.13.0 路径或禁用 Qwen3.6 实例。
