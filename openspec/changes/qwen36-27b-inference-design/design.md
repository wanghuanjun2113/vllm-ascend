...
总体实现思路是把性能目标拆到 910 和 310 两条硬件路径上分别闭环。910 侧以关键特性使能为主，优先组合 W8 动态量化、ACLGraph Full decode、Chunk Prefill、Prefix Caching 和 MTP，目标是在硬件能力充足的场景下压缩 prefill、decode 和采样链路的端到端时延。310 侧分两步推进：第一步补齐 GDN 和 MRoPE 等关键算子，先实现 Qwen3.6-27B 的基本推理性能；第二步在算子路径稳定后，再使能 Chunk Prefill、Prefix Caching、MTP 等关键特性，进一步提升长上下文和 decode 性能。

主要风险来自 think token 膨胀。与 Qwen32B 相比，Qwen3.6 的 think 过程预计需要约 4 倍 token，这会直接放大端到端时延压力。即使使能 MTP、Prefix Caching、ACLGraph 等加速特性，端到端时延相对 Qwen32B 仍可能存在 30%～50% 差距；因此性能验收必须同时记录生成 token 数、think token 占比、TTFT、TPOT 和端到端耗时，不能只看单项算子或单 token 指标。
