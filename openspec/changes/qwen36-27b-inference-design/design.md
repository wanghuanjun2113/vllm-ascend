...
- 310P GDN 当前走 eager fallback：`vllm_ascend/_310p/ops/fla/gdn_310.py` 调用 `fused_gdn_gating_pytorch`、`chunk_gated_delta_rule_pytorch` 和 `fused_recurrent_gated_delta_rule_pytorch`。
- `chunk_gated_delta_rule_pytorch` 位于 `vllm_ascend/_310p/ops/fla/chunk_gated_delta_rule.py`，当前实现为 PyTorch fallback，内部包含 chunk size 64、变长 TND 输入归一化、float32 recurrent state 计算。
...
- 明确 0.13.0/0.18.0 双版本路由和 RTSP 包选择规则。
- 明确 0.18.0 `netrsnpython3rdadvance` 的运行态无 GCC 依赖验收方式。
- 明确 W8 动态量化、Prefix Cache、MTP、ACLGraph Full decode、Chunk Prefill、CoT 控制的配置边界。
- 明确 310P 必补算子、fallback 边界和性能占比约束。
...
替代方案：在启动时按机器环境即时编译自定义算子。该方案违反 0.18.0 GCC 安全红线，不采用。
...
| 算子 | 当前锚点或参考 | 目标 |
|---|---|---|
| `chunk_gated_delta_rule_fwd` | `_310p/ops/fla/chunk_gated_delta_rule.py` PyTorch fallback | AscendC prefill 主路径 |
| `rmsnormgated` | `_310p/ops/layernorm.py` 和通用 RMSNorm 路径 | RMSNorm + gate fused |
| `fused_gdn_gating` | `_310p/ops/fla/fused_gdn_gating.py` PyTorch fallback | exp/softplus/sigmoid fused |
...
新增算子整网耗时占比相对 910 不得超过 30%。该指标按端到端 Profiling 归因，不按单算子 micro benchmark 代替。
...
替代方案：310P 直接对全模型做 graph capture。当前 fallback 中 GDN recurrent state、conv state、变长 metadata 和 PyTorch fallback 均可能破坏 graph 稳定性，不采用。
...
- Decode：ACLGraph replay/eager fallback、GDN recurrent update、通信。
...
- 缓存：KV block、Prefix Cache hit、SSM state checkpoint 或 fallback。
...
- [0.18.0 运行态 GCC 红线] -> 所有 C++/AscendC/Triton 编译动作前移到构建期；运行态扫描作为发布门禁。
- [310P 算子周期长] -> 先交付 eager 正确性和 profiling，再按耗时占比开发 `chunk_gated_delta_rule_fwd`、`split_qkv_rmsnorm_Mrope` 等高优先级算子。
...
1. 新建 0.18.0 Qwen3.6 专用镜像和 `netrsnpython3rdadvance` 包，不影响 0.13.0 既有模型路径。
2. 在灰度环境按 CR `spec.vllmVersion=0.18.0` 路由 Qwen3.6 实例，保留 `0.13.0` 回退路径。
3. 先启用 FP16 eager，随后逐项启用 W8 动态量化、Chunk Prefill、ACLGraph、Prefix Cache、MTP；每次只打开一个新增变量。
4. 若 0.18.0 运行态依赖或性能不达标，CR 回退到不承载 Qwen3.6 的 0.13.0 路径或禁用 Qwen3.6 实例。
