...
系统 SHALL 支持 Qwen3.6-27B 在 910B4 32G x4、TP=4、并发 4 条件下运行。

#### Scenario: 910 标准实例启动
- **WHEN** 使用 910B4 x4 启动 Qwen3.6-27B 标准实例
- **THEN** 系统 SHALL 使用 TP=4，并在动态量化配置下于 8 分钟内完成启动就绪。

### Requirement: 910 关闭 Prefix Cache 性能基线
系统 SHALL 在关闭 Prefix Cache 的条件下满足 910 性能基线。

#### Scenario: 910 长上下文基线
- **WHEN** Prefix Cache 关闭、并发为 4、输入长度为 32K、输出长度为 10K
