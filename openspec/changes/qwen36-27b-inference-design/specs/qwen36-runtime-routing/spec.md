...
#### Scenario: 选择 Qwen3.6 0.19.1+ 路径
- **WHEN** 产品 CR 声明 Qwen3.6-27B 使用高版本 vLLM/vllm-ascend 路径
- **THEN** 系统 SHALL 选择 0.19.1+ vLLM/vllm-ascend 版本组合和 `netrsnpython3rdadvance` RTSP 包，并允许承载 Qwen3.6-27B。
...
- **THEN** 运维或测试 SHALL 能从日志或状态中确认该实例使用 0.19.1+ 版本组合和 `netrsnpython3rdadvance` 包。
