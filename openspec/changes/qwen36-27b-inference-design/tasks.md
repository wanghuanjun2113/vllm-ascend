...
- [ ] 2.2 定义 Qwen3.6-27B 到 0.19.1+ vLLM/vllm-ascend、`netrsnpython3rdadvance` 和 Qwen3.6 镜像的路由规则，并记录 `v0.19.1rc1` RC 版本风险
- [ ] 2.3 在外部微服务或部署层输出最终 vLLM 版本、vllm-ascend 版本、RTSP 包名和镜像标签
- [ ] 2.4 为 Qwen3.6 0.19.1+ 版本线构建预编译 wheel、AscendC `.so` 和必要 Triton cache，确保运行态不触发源码编译
- [ ] 2.5 对 Qwen3.6 0.19.1+ 版本线镜像或 RTSP 包执行运行态依赖扫描，证明无 GCC 依赖
