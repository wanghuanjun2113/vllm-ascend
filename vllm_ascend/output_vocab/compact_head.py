...
COMPACT_OUTPUT_HEAD_FAST_GREEDY_ENV = "VLLM_ASCEND_COMPACT_OUTPUT_HEAD_FAST_GREEDY"
...
class CompactLogitsProcessor(LogitsProcessor):
    def __init__(
        ...
        return_compact_logits: bool = False,
    ) -> None:
        super().__init__(original_vocab_size)
        self.compact_vocab_size = compact_vocab_size
        self.return_compact_logits = return_compact_logits
    ...
    def _get_logits(
        ...
        with record_function_or_nullcontext("compact_lm_head_gemm"):
            compact_logits = lm_head.quant_method.apply(
                lm_head,
                hidden_states,
                bias=embedding_bias,
            )
        with record_function_or_nullcontext("compact_tp_gather"):
        ...
        if self.return_compact_logits:
            return compact_logits
        return self.expand_logits(compact_logits)

    def expand_logits(self, compact_logits: torch.Tensor) -> torch.Tensor:
        full_shape = (*compact_logits.shape[:-1], self.vocab_size)
        with record_function_or_nullcontext("compact_full_logits_scatter"):
...
class CompactOutputVocabState:
    ...
    compact_vocab_size: int
    compact_to_original_ids: torch.Tensor
    original_to_compact_ids: torch.Tensor
...
def apply_compact_output_head(
    ...
    original_to_compact_ids = torch.full(
        (original_vocab_size,), -1, dtype=torch.int64, device=device
    )
    original_to_compact_ids[mapping_tensor] = torch.arange(
        compact_vocab_size, dtype=torch.int64, device=device
    )
    return_compact_logits = os.getenv(COMPACT_OUTPUT_HEAD_FAST_GREEDY_ENV, "").strip() == "1"
    processor = CompactLogitsProcessor(
        original_vocab_size,
        compact_vocab_size,
        mapping_tensor,
        return_compact_logits=return_compact_logits,
    ).to(device)
